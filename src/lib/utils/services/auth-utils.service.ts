...
export class AuthUtilsService {
  ...
  async sanitizeUser<T = UserResponseDto>(
    user: User & { profilePicture?: FileInstance | string | null },
  ): Promise<T> {
    if (!user) return null as T;

    // Handle profilePicture as either FileInstance object or string URL
    const profilePictureUrl = user?.profilePicture
      ? typeof user.profilePicture === 'string'
        ? user.profilePicture
        : ((user.profilePicture as FileInstance).url ?? null)
      : null;
